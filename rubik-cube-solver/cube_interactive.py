...
class Cube:
    ...
    def __init__(self, N=3, plastic_color=None, face_colors=None, faces_array: List[List] = None):
        self.N = N
        ...
        self._move_list = []
        self._initialize_arrays(faces_array)

    def _initialize_arrays(self, faces_array: List[List]):
        ...
        for i in range(6):
            ...
            if faces_array is None:
                ...
                colors_i = np.array(faces_array[i])
...
class InteractiveCube(plt.Axes):
    def __init__(self, cube=None,
        ...
        self.figure.text(0.05, 0.05,
                         "Mouse/setas do teclado para rotacionar o cubo\n"
                         "U/D/L/R/B/F para rotacionar faces\n"
                         "U - TOPO, D - BASE\nL - ESQUERDA, R - DIREITA\nB - TRAS, F - FRENTE\n"
                         "(segure shift para movimentos anti-horarios)",
                         size=10)

    def _initialize_widgets(self):
        self._ax_reset = self.figure.add_axes([0.75, 0.05, 0.2, 0.075])
        self._btn_reset = widgets.Button(self._ax_reset, 'Resetar perspectiva')
        self._btn_reset.on_clicked(self._reset_view)

        self._ax_solve = self.figure.add_axes([0.55, 0.05, 0.2, 0.075])
        self._btn_solve = widgets.Button(self._ax_solve, 'Resolver cubo')
